...
if ( !function_exists ( 'bs_get_gallery_ids' ) ) {
	function bs_get_gallery_ids() {
		global $post;
		$id_array = '';
		$postid = $post->ID;
...
// Get attachment data
if ( !function_exists ( 'bs_get_attachment' ) ) {
	function bs_get_attachment( $attachment_id ) {
...
if ( !function_exists ( 'bs_gallery_count' ) ) {
	function bs_gallery_count() {
...
function bs_gallery_is_lightbox_enabled() {
	$link_images = get_post_meta( get_the_ID(), '_easy_image_gallery_link_images', true );
